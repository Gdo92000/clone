---
title: Session Memory — Flux Delivery (arquivado)
updated: 2026-05-22T21:37:00-03:00
tags:
- type/source
- type/archive
status: archived
supersedes: docs/obsidian/MEMORY.md
---

# 🧠 Session Memory — Flux Delivery

> Para uso em **outro chat** e visível no **Obsidian** via [[Session Memory]].
> Tudo que um novo agente precisa para continuar o trabalho sem perder contexto.
...
## 11. Proximity Feature — Correções e Testes (2026-05-23)

### Seed bug corrigido

`seedFromRestaurants()` em `server/src/services/coverageCityService.ts` agora computa lat/lng corretamente a partir das coordenadas dos restaurantes (média do grupo por cidade), em vez de inserir `'0','0'`.

### Testes adicionados

| Arquivo | Testes | O que cobre |
|---------|--------|-------------|
| `server/src/routes/routes.test.ts` | +11 (total 24) | Coverage cities CRUD admin: GET /:id, POST, PUT, PATCH toggle, DELETE, POST /seed + edge cases (404, 400) |
| `src/services/locationService.test.ts` | 8 novos | `calculateDistance()` Haversine (5 casos), `formatDistance()` (3 casos) |
| `src/services/cityCoverageService.test.ts` | 13 novos | `normalizeCityName()` (4), `isSameCityName()` (4), `getRegisteredCityCoverages()` (1), `findRegisteredCityCoverage()` (4) |
...
| `vitest run` | ✅ 242 testes (17 suítes) |
| `vite build` | ✅ limpo |
| `eslint` backend (coverageCityService) | ✅ 0 erros |
...
| ✅ 24 | Proximity — Correções e Testes | Concluída |

---

> **Última atualização**: 2026-05-23T07:50:00-03:00  
> **Branch**: `main`
