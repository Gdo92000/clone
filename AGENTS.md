...
```
docs/obsidian/
├── _index.md          # Auto-indexação de todas as notas
├── MEMORY.md          # Memória enxuta — estado atual + progresso consolidado
├── CURRENT_STATE.md   # Estado operacional — fase, commit, bloqueios
├── worklog/           # Notas temporárias (tarefas, sessões, checkpoints, bugs)
│   └── _index.md
├── knowledge/         # Conhecimento permanente (arquitetura, padrões, integrações)
│   └── _index.md
├── adr/               # Architectural Decision Records (decisões permanentes)
│   └── _index.md
├── mocs/              # MOCs (Maps of Content) — hubs semânticos
│   ├── MOC Architecture.md
│   ├── MOC Frontend.md
│   ├── MOC Backend.md
│   ├── MOC Auth.md
...
└── archive/           # Notas antigas arquivadas — NUNCA deletar
    └── _index.md
```

Ao criar ou editar notas no Obsidian, carregar a skill "obsidian-markdown" para garantir sintaxe correta (callouts, wikilinks, propriedades, etc.).
