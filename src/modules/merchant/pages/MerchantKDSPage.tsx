...
const COLUMNS: { id: string; label: string; statuses: MerchantOrderStatus[]; color: string }[] = [
...
export function MerchantKDSPage() {
  const { data: orders = [], isLoading, isError, error } = useKdsOrders();
  ...
  const updateStatus = useCallback(
    (orderId: string, status: MerchantOrderStatus) => {
      void updateOrderStatusAsync({ orderId, status });
    },
        ...
        {columns.map((col) => (
          <section key={col.id} className="flex flex-col gap-3">
            ...
            {col.orders.map((order) => (
              <article
                key={order.id}
                className={`rounded-xl border border-border-default bg-surface-elevated p-4 border-l-4 ${col.color}`}
              >
