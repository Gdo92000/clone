...
describe('Serializer', () => {
  describe('serializeEntity', () => {
    ...
    it('converte NaN → "__NaN__"', () => {
      ...
      expect((fixture as Record<string, unknown>).value).toBe('__NaN__');
    });

    it('converte Infinity → "__Infinity__"', () => {
      const fixture = serializeEntity({ id: '1', value: Infinity });
      expect((fixture as Record<string, unknown>).value).toBe('__Infinity__');
    });
  ...
  describe('assertSerializable', () => {
    ...
    it('sanitiza referência circular sem StackOverflow', () => {
      ...
      const obj: any = { id: '1', selfRef: 'going-back', self: null as unknown as any };
      obj.self = obj;
